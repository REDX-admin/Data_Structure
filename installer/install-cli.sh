...
update_initramfs() {
	...
	if [ -d "$DRACUT_CONF_DIR" ]; then
		# Add prl_tg/vid/eth modules to initramfs and exclude other.
		echo 'add_drivers+="prl_tg prl_vid prl_eth"' > "$DRACUT_KMODS_FILE"
		echo 'omit_drivers+="prl_fs prl_fs_freeze"' >> "$DRACUT_KMODS_FILE"
		# Update initramfs.
		dracut -f

	# Debian-based
	elif type update-initramfs > /dev/null 2>&1; then
		# Add prl_tg/vid/eth modules to initramfs
